...
{
	// ajout d'un compteur permettant a la fonction de s'arreter aprËs avoir rÈcupÈrÈ $stop items
	$cpt=0;
   ...
   // Ouverture du repertoire demandÈ
   $handle = opendir($dir);
...
{/*MÈthode qui permet de dÈcompresser un fichier zip $file dans un rÈpertoire de destination $path
   et qui retourne un tableau contenant la liste des fichiers extraits                                                                                ?> Si $effacer_zip est Ègal ‡ true, on efface le fichier zip d'origine $file*/
   ...
   if($zip)
   {
      while($zip_entry = zip_read($zip))//Pour chaque fichier contenu dans le fichier zip
      {
         if(zip_entry_filesize($zip_entry) > 0)
         {
            ...
            $nom_fichier = strtr($nom_fichier, "¿¡¬√ƒ≈‡·‚„‰Â“”‘’÷ÿÚÛÙıˆ¯»… ÀËÈÍÎ«ÁÃÕŒœÏÌÓÔŸ⁄€‹˘˙˚¸ˇ—Ò", "AAAAAAaaaaaaOOOOOOooooooEEEEeeeeCcIIIIiiiiUUUUuuuuyNn");
